  ...
  try {
    ...
    for (const statement of statements) {
      try {
        ...
        if (error) {
          console.log('[v0] RPC not available, using direct SQL approach...');
          // RPC doesn't exist, fall back to direct SQL execution
          ...
          if (!response.ok && response.status !== 404) {
            console.warn(`[v0] Statement failed: ${statement.substring(0, 50)}...`);
          }
        }
      } catch (err) {
        console.log(`[v0] Skipping statement (may already exist): ${statement.substring(0, 30)}...`);
      }
    }
    ...
    console.error('[v0] Failed to migrate:', error.message);
    process.exit(1);
  }
}
